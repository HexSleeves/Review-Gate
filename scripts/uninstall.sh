...
REVIEW_GATE_DIR="$HOME/cursor-extensions/review-gate-v3"
...
if [[ -f "$CURSOR_MCP_FILE" ]]; then
    ...
    # Remove review-gate-v3 entry (simple approach - remove entire config)
...
if command -v cursor &> /dev/null; then
    ...
    if cursor --uninstall-extension review-gate-v3 >/dev/null 2>&1; then
