...
timeout 5s python review_gate_v3_mcp.py > "$TEMP_DIR/mcp_test.log" 2>&1 || true
deactivate
...
EXTENSION_FILE="$ROOT_DIR/V3/review-gate-v3-0.0.1.vsix"
if [[ -f "$EXTENSION_FILE" ]]; then
    ...
    if [[ "$EXTENSION_INSTALLED" == false ]]; then
        ...
        log_step "4. Select: $REVIEW_GATE_DIR/review-gate-v3-2.7.3.vsix"
...
if [[ -f "$ROOT_DIR/ReviewGate.mdc" ]] && [[ -n "$CURSOR_RULES_DIR" ]]; then
    log_progress "Installing global rule..."
    mkdir -p "$CURSOR_RULES_DIR"
    cp "$ROOT_DIR/ReviewGate.mdc" "$CURSOR_RULES_DIR/"
    log_success "Global rule installed to: $CURSOR_RULES_DIR"
elif [[ -f "$ROOT_DIR/ReviewGate.mdc" ]]; then
    log_warning "Could not determine Cursor rules directory for this platform"
    log_info "Global rule available at: $ROOT_DIR/ReviewGate.mdc"
fi
...
if [[ -f "$REVIEW_GATE_DIR/review_gate_v3_mcp.py" ]] && \
   [[ -f "$CURSOR_MCP_FILE" ]] && \
